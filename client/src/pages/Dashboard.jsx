import React, { useEffect, useState } from 'react'
import { dummyCreationData } from '../assets/assets'
import { Gem, Sparkles } from 'lucide-react'
import { useUser } from '@clerk/react'
import CreationItem from '../components/CreationItem'

const Dashboard = () => {
  const { user } = useUser()
  const [creation, setCreations] = useState([])
  const planLabel = user ? 'Premium' : 'Free'

  const getDeshboardData = async () => {
    setCreations(dummyCreationData)
  }

  useEffect(() => {getDeshboardData()}, [])
  return (
    <div className='h-full overflow-y-scroll p-6'>
      <div className='flex justify-start gap-4 flex-wrap'>
        {/* Total Creation card */}
        <div className='flex justify-between items-center w-72 p-4 px-6 bg-white 
        rounded-xl border border-gray-200'>
          <div className='text-slate-600'>
            <p className='text-sm '> Total Creation </p>
            <h2 className='text-xl font-semibold'>{creation.length}</h2>

          </div>
          <div className='w-10 h-10 rounded-lg bg-gradient-to-r from-[#3588F2] to-[#0BB0D7]
           flex items-center justify-center text-white'>
            <Sparkles className='w-5 text-white'/>
          </div>
        </div>

        {/* Active plan card */}
        <div className='flex justify-between items-center w-72 p-4 px-6 bg-white rounded-xl border border-gray-200'>
          <div className='text-slate-600'>
            <p className='text-sm'> Active Plans </p>
            <h2 className='text-xl font-semibold'>
              {planLabel}
            </h2>
          </div>
          <div className='w-10 h-10 rounded-lg bg-gradient-to-r from-[#FF61C5] to-[#9E53EE] flex items-center justify-center text-white'>
            <Gem className='w-5 text-white'/>
          </div>
        </div>

      </div>

      <div className='space-y-3'>
        <p className='mt-6 mb-4'>Recent Creations</p>
        {
          creation.map((item) => (
            <CreationItem key={item.id} item={item} />
          ))
        }
      </div>

     
    </div>
  )
}

export default Dashboard
